...
def main() -> None:
    ...
    with VideoCard() as vc:
        with VideoRecorder(f"{FILE_NAME}.mp4", vc) as vr:
            with AudioInput(input_name="Pyle") as ai:
                with AudioRecorder(ai, filename=f"{FILE_NAME}.wav") as ar:
                    while True:
    ...
    vu.combine_video_and_audio(f"{FILE_NAME}.mp4", f"{FILE_NAME}.wav", f"{FILE_NAME}.mkv")
    log.info(f"Process complete. Results stored in {FILE_NAME}.mkv")
